...
$input_ref->{'Namespace'} = "System/Freebsd";
...
=cut
	my $mfree = `/sbin/sysctl -n vm.stats.vm.v_free_count`;
	my $mtotl = `/sbin/sysctl -n hw.realmem`;
	my $mbuff = `/sbin/sysctl -n vfs.bufspace`;
	my $mcach = `/sbin/sysctl -n vm.stats.vm.v_cache_count`;
	my $page_size = `/sbin/sysctl -n vm.stats.vm.v_page_size`;
	my $macti = `/sbin/sysctl -n vm.stats.vm.v_active_count`;
	my $minac = `/sbin/sysctl -n vm.stats.vm.v_inactive_count`;
	chomp($mbuff, $mtotl, $page_size, $mcach, $macti, $minac);
	$mcach = $page_size * $mcach;
	$macti = $page_size * $macti;
	$minac = $page_size * $minac;

	my $mem_total = $mtotl;
	my $mem_free = $mfree;
	my $mem_cached = $mcach;
	my $mem_buffers = $mbuff;
